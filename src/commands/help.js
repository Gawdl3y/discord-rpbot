...
export default {
	...
	run(message, matches) {
		if(matches[1]) {
			...
			const command = commands.find(c => c.information.name === lowercaseSearch || (c.information.aliases && c.information.aliases.includes(lowercaseSearch)));
			if(command) {
			...
			const commandList = commands.filter(c => c.isRunnable(message)).map(c => c.information.name + ' - ' + c.information.description).join('\n');
